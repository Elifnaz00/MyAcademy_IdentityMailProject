...
namespace IdentityMail.Web.Controllers
{
    public class DraftController(UserManager<AppUser> _userManager, AppDbContext _context) : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Create(DraftMailDto draftMailDto)
        {
            ...
            if (receiver is null)
            {
                ModelState.AddModelError("", "Girdiğiniz mail ile sistemde kayıtlı bir kullanıcı bulunamadı");
                return View(draftMailDto);
            }

            var draft=new UserMessage
            {
                Subject = draftMailDto.Subject,
                Body = draftMailDto.Body,
                SenderId = sender.Id,
